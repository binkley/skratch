...
internal class ValueSetMockTest {
    ...
    @Test
    fun shouldAddNew() {
        val entry = spyk(ValueEntry(key, RuleValue { _, _ -> 3 }))
    ...
    fun shouldReplaceWhenAddingSameKey() {
        val entry = spyk(ValueEntry(key, RuleValue { _, _ -> 3 }))
        val set = ValueSet(layer, mutableSetOf(entry))
        val other = spyk(ValueEntry(key, spyk(RuleValue { _, _ -> 4 })))

        set.add(other)

        assertEquals(1, set.size)

        verify(exactly = 1) { entry.replaceWith(layer, other) }
        verify(exactly = 1) { other.add(layer) }
    }
    ...
    fun shouldRemoveWhenAddingNothing() {
        ...
        verify(exactly = 1) { entry.remove(layer) }
    }
    ...
    fun shouldNotAddDuplicate() {
        ...
        val entry = spyk(ValueEntry(key, rule))
        val set = ValueSet(layer, mutableSetOf(entry))
        ...
        verify(exactly = 0) { entry.replaceWith(layer, other) }
    }
    ...
    fun shouldNotRemoveWhenAbsent() {
        ...
        verify(exactly = 0) { entry.remove(layer) }
    }
}
