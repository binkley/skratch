...
abstract class GeneralMatrix<N, Norm : GeneralNumber<Norm, Norm>, M>(
    ...
    override fun plus(other: M) = matrixCtor(values.zip(other.values) { a, b -> a + b })
    ...
    private inner class Col(
        n: Int
    ) : List<N> by col(n)

    private val Int.asColNumber get() = (this % cols) + 1

    private fun col(col: Int) =
        values.filterIndexed { index, _ ->
            col == index.asColNumber
        }

    private operator fun Row.times(col: Col) =
        this
            .zip(col)
            .map { (r, c) -> r * c }
    ...
    fun equivalent(other: GeneralMatrix<*, *, *>) =
        values.size == other.values.size &&
            values
                .zip(other.values) { a, b -> a.equivalent(b) }
                .reduce { acc, it -> acc && it }
}
