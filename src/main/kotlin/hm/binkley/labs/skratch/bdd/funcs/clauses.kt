...
internal infix fun Scenario.`A snack helps with thinking`(GIVEN: Given) = act { }

internal infix fun Scenario.`A revolution is missed`(GIVEN: Given) = act { }

internal infix fun Given.`an apple`(WHEN: When) =
    act {
        apple = Apple(Newton(thinking = false, eatingApple = false))
    }

internal infix fun When.`it falls`(THEN: Then) =
    act {
        apple.falls()
    }

internal infix fun Then.`Newton thinks`(QED: QED) =
    act {
...
internal infix fun Then.`Newton eats the apple`(QED: QED) =
    act {
...
internal infix fun Then.`Newton sleeps`(QED: QED) =
    act {
